...
local function UpdateResults()
    ...
    local needItemRows = math.max(NUM_ROWS, nItems)
    ...
    if nItems > 0 then
        ...
        for i = 1, needItemRows do
            if not resultRows[i] then
                resultRows[i] = createItemRow()
            end
            local entry = itemList[i]
            local row = resultRows[i]
            local rowY = (i <= nItems) and (currentY - (i - 1) * ROW_HEIGHT) or (currentY - (nItems - 1) * ROW_HEIGHT)
            row:ClearAllPoints()
        ...
        -- Group overlays for Total column
    ...
    if nRecipes > 0 then
        ...
        for i = 1, nRecipes do
            if not recipeRows[i] then
                recipeRows[i] = createRecipeRow()
            end
            local entry = recipeList[i]
            local row = recipeRows[i]
            local rowY = currentY - (i - 1) * ROW_HEIGHT
            row:ClearAllPoints()
            row:SetPoint("TOPLEFT", resultsArea, "TOPLEFT", 0, rowY)
            row:SetPoint("TOPRIGHT", resultsArea, "TOPRIGHT", 0, rowY)
            row:SetPoint("BOTTOMLEFT", resultsArea, "TOPLEFT", 0, rowY - ROW_HEIGHT)
        ...
        for i = nRecipes + 1, #recipeRows do
            recipeRows[i]:Hide()
            recipeRows[i].entry = nil
        end
