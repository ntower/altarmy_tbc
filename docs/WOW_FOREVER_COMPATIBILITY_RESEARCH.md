...
| Item Info / Spell Info | Cross-cutting, ~20 files (`Data/Gear/ItemStats.lua`, `ItemUsability.lua`, `GearScore.lua`, `GearUpgrade*.lua`, `GearCompare.lua`, `DataStoreProfessions.lua`, `Data/Search/*`, `GuildTabData.lua`, several `Tabs/*`/`UI/*`) | No dedicated Thaoky module — same ad-hoc inline lookups we already use, seen in `DataStore_Containers.lua`/`DataStore_Spells.lua` | `C_Item.GetItemInfo`/`GetItemInfoInstant`/`GetItemStats`, `C_Spell.GetSpellName`/`GetSpellInfo`(table return)/`GetSpellLink` | Highest fan-out; mechanical once call shapes are confirmed — plan is a small number of shared compat helpers, not 20 inline patches | **Done** for `GetItemInfo` call sites (see Phase B below); `GetSpellInfo`/`GetItemStats`/`IsUsableItem` call sites outside `DataStoreProfessions.lua` still open. **Still true as of the "Stat weights for WoW Forever" entry below**: `ItemStats.lua`'s `fetchFromApi` still calls the raw `GetItemStats` global directly, never migrated onto the existing (unused) `DS.CompatGetItemStats`/`C_Item.GetItemStats` fallback, despite the "Full UI/presentation layer migrated" entry further down claiming `ItemStats.lua` was swept — confirmed `GetItemStats` is 0/7 present on Forever per the apicheck results above, so Forever item stats today come from tooltip-regex parsing only. Flagged, not fixed here (cross-cutting, out of scope for that task). |
...
## Stat weights for WoW Forever (2026-09-19)

Gear-upgrade scoring (`Data/Gear/PawnScales.lua`, referenced above under "Domain data, separate from API compatibility") is TBC-itemization-specific in one concrete way this doc hadn't yet detailed: TBC's HawsJon-sourced Pawn scale weights hit and crit as two separate stats each (physical `HitRating`/`CritRating` vs spell `SpellHitRating`/`SpellCritRating`), because TBC gear actually rolls them separately. WoW Forever, being vanilla-itemization-based, has no such split — just one Hit and one Crit stat — so using the TBC weight table as-is on Forever would misjudge every hit/crit-bearing item (undercounting whichever of the two TBC weights was smaller).

**Implemented:** a new, fully independent scale table, [`Data/Gear/PawnScalesForever.lua`](../AltArmy_TBC/Data/Gear/PawnScalesForever.lua) (`AltArmy.PawnScalesForever`, own `COMMON`/`RAW`/`LEVEL_OVERRIDES`/`GetRawScale`/`GetOverrideBandId` — not derived from `PawnScales.lua` at load time, per explicit requirement that the two configs stay independently adjustable). Its values are mechanically derived from `PawnScales.RAW`: for every class/spec, `HitRating = SpellHitRating = max(TBC HitRating, TBC SpellHitRating)` and `CritRating = SpellCritRating = max(TBC CritRating, TBC SpellCritRating)`, every other stat copied unchanged. Writing the merged value into *both* the physical and spell key names (rather than picking one) is a deliberate hedge: it isn't confirmed whether Forever's item data reports hit/crit under the physical key, the spell key, or a unified one (see the "Item Info / Spell Info" roadmap row above, which this entry also updates — `ItemStats.lua`'s `GetItemStats` path is confirmed dead on Forever, so today this actually resolves through tooltip-regex parsing, whose exact wording on Forever is equally unconfirmed). Populating both keys means scoring is correct regardless of which one real Forever gear/tooltips use.

Version selection: `AltArmy.DataStore.IsWowForever` (`DataStore.lua`, next to `MAX_LEVEL`) — `(GetBuildInfo and select(4, GetBuildInfo()) == 16001) or false`. This is the one place in the stat-weight/scoring code that genuinely needs a version check rather than an existence check (per this doc's own "existence checks over version checks wherever possible" convention) — there's no API to defensively probe when the choice is "which entire data table," only when calling a function that might not exist. `GearUpgrade.lua` picks between the two scale modules via a small `activeScaleModule()` helper at its two `PS.GetRawScale`/`PS.GetOverrideBandId` call sites.

**Deliberately not done:** wand-leveling `LEVEL_OVERRIDES` (the numbers that boost `RangedDps`/`MeleeDps` while leveling, since wand damage matters leveling but not at endgame) are copied verbatim from TBC in `PawnScalesForever.lua`, not re-tuned. The user has previously observed HawsJon under-weights wand damage relative to how much it matters while leveling (this addon is used mostly while leveling); retuning is real, playtested-by-feel work with no numbers to guess at yet, so it's left as a follow-up (marked with a `TODO` comment in the file) rather than invented here.

## SavedVariables not loading after a cold client start — client-wide beta bug, not ours (2026-09-19)

Investigated the user's report that "addons can't save data between sessions" on Forever. Confirmed via multiple independent community reports that this is a **Forever-beta-wide client bug**, not something specific to AltArmy_TBC or any single addon — not yet reproduced against our own addon in-game, but the mechanism described would affect every addon identically, ours included.

**Symptom:** account/character SavedVariables files under `WTF\Account\<account>\...` are written correctly to disk on logout/`/reload` (multiple testers confirmed the file contents on disk are exactly right), but are not read back in on a full client restart (cold start) — a load-side bug, not a save-side one. `/reload` alone can appear to preserve data in some tests, which misled at least two testers before they checked a real exit/relaunch. Retail, Era, and MoP Classic clients on the same machines save normally — isolated to the Forever client specifically.

**Blizzard status: not acknowledged, no timeline.** Checked Blizzard's official "Known Issues" threads for the Forever beta dated Sept 17 and Sept 18 2026 (including CM Kaivax's own reply in the latter) — neither mentions SavedVariables or addon-settings persistence, despite an active official-forum thread ([UI/Addon settings wiped on client restart](https://us.forums.blizzard.com/en/wow/t/uiaddon-settings-wiped-on-client-restart/2353992)) and a community bug-tracker issue ([`ClassicWoWCommunity/forever-bugs#34`](https://github.com/ClassicWoWCommunity/forever-bugs/issues/34)) already documenting it as of build 69913.

**Community-tested workarounds, per `forever-bugs#34`:**
- **CVars do not work**, despite appearing to — `SetCVar`/`GetCVar` survive `/reload` (a UI reload never clears memory) but are never written to `config-cache.wtf`, so they're gone after a real restart. At least two testers were initially fooled by this because they only verified with `/reload`.
- **Macro bodies do survive full restarts** — macros live server-side on the account, bypassing the broken file-load path entirely. Confirmed working by two independent testers. Constraints: 255 characters per macro, a limited macro count, the macro list isn't populated yet at `ADDON_LOADED` (arrives from the server slightly later — must wait for `UPDATE_MACROS` or poll), and macros can't be written while in combat.
- **A larger-capacity workaround exists but needs a script outside the game:** since the client still runs ordinary addon Lua files at login (just not the SavedVariables ones), one tester built an external script that copies each addon's `WTF\...\SavedVariables\<Addon>.lua` into a small "seed" `.lua` file, loaded via a `!!`-prefixed loader addon listing that seed directly in its own `.toc` (so it loads as plain addon code, not as a SavedVariables load). This restores full-size data but requires a companion script running outside WoW plus a relog (not just `/reload`) whenever a new addon is added to the bridge.

**Decision: no code change to AltArmy_TBC.** Given how much data this addon stores (characters, gear, professions, auctions, reputations, etc. across the whole account), neither community workaround is viable to build in: the 255-char macro trick is far too small for our data volume, and the external-script bridge asks end users to run something outside the game and is fragile (silent staleness if the bridge script isn't running). This is a client bug, not an addon-architecture problem — same treatment this doc already gives the still-unresolved minimap-position-save bug above. Logged here so it isn't mistaken for an AltArmy_TBC-specific bug if a user reports lost data, and to revisit once Blizzard ships a fix or acknowledges it (neither has happened yet).
...
- [`ClassicWoWCommunity/forever-bugs#34`](https://github.com/ClassicWoWCommunity/forever-bugs/issues/34) — community bug tracker issue on the SavedVariables cold-start load bug; source of the CVar/macro/seed-file workaround findings above
