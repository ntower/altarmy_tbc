...
describe("DataStoreReputations", function()
  ...
  describe("FormatReputationProgressText", function()
    it("returns current over next in K, rounded to nearest 1k", function()
      assert.are.equal("14k/42k", DS.FormatReputationProgressText("Friendly", 14000, 42000))
    end)
    it("rounds to the nearest 1k", function()
      assert.are.equal("29k/36k", DS.FormatReputationProgressText("Hated", 29050, 36000))
      assert.are.equal("1k/1k", DS.FormatReputationProgressText("Exalted", 842, 1000))
    end)
    it("returns cur/1k for Exalted when span is zero (capped / API)", function()
      assert.are.equal("1k/1k", DS.FormatReputationProgressText("Exalted", 500, 0))
    end)
  ...
  describe("FormatReputationPercentText", function()
    it("rounds the rate to the nearest 1 percent", function()
      assert.are.equal("81%", DS.FormatReputationPercentText(80.7))
      assert.are.equal("80%", DS.FormatReputationPercentText(80.4))
      assert.are.equal("0%", DS.FormatReputationPercentText(0))
      assert.are.equal("100%", DS.FormatReputationPercentText(100))
    end)
    it("clamps out-of-range rates to 0..100", function()
      assert.are.equal("0%", DS.FormatReputationPercentText(-5))
      assert.are.equal("100%", DS.FormatReputationPercentText(150))
    end)
    it("treats a nil rate as 0 percent", function()
      assert.are.equal("0%", DS.FormatReputationPercentText(nil))
    end)
    it("shows Max instead of 100% when Exalted is maxed", function()
      assert.are.equal("Max", DS.FormatReputationPercentText(100, "Exalted"))
      assert.are.equal("Max", DS.FormatReputationPercentText(99.7, "Exalted"))
    end)
    it("shows the percent for Exalted that is not yet maxed", function()
      assert.are.equal("42%", DS.FormatReputationPercentText(42, "Exalted"))
    end)
    it("shows 100% for non-Exalted standings at full", function()
      assert.are.equal("100%", DS.FormatReputationPercentText(100, "Honored"))
    end)
  end)

  describe("FormatReputationProgressTextExact", function()
    it("returns the exact current over next, no abbreviation", function()
      assert.are.equal("29050/36000", DS.FormatReputationProgressTextExact("Hated", 29050, 36000))
      assert.are.equal("14000/42000", DS.FormatReputationProgressTextExact("Friendly", 14000, 42000))
    end)
    it("returns cur/1000 for Exalted when span is zero (capped / API)", function()
      assert.are.equal("999/1000", DS.FormatReputationProgressTextExact("Exalted", 999, 0))
    end)
    it("returns Max when nextLevel is zero or negative", function()
      assert.are.equal("Max", DS.FormatReputationProgressTextExact("Honored", 100, 0))
      assert.are.equal("Max", DS.FormatReputationProgressTextExact("Honored", 100, -1))
    end)
  end)
