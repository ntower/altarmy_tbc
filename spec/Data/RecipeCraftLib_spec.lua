...
describe("RecipeCraftLib", function()
  ...
  describe("PickHardestDifficulty", function()
    it("returns nil for empty or unknown values", function()
      assert.is_nil(RCL.PickHardestDifficulty(nil))
      assert.is_nil(RCL.PickHardestDifficulty({}))
      assert.is_nil(RCL.PickHardestDifficulty({ nil, "unknown" }))
    end)

    it("picks orange over yellow/green/gray", function()
      assert.are.equal("orange", RCL.PickHardestDifficulty({ "gray", "orange", "yellow" }))
      assert.are.equal("yellow", RCL.PickHardestDifficulty({ "green", "yellow", "gray" }))
      assert.are.equal("green", RCL.PickHardestDifficulty({ "gray", "green" }))
      assert.are.equal("gray", RCL.PickHardestDifficulty({ "gray" }))
    end)
  end)

  describe("FormatCollapsedSkillCell", function()
    it("returns * when recipe level unknown", function()
      assert.are.equal("*", RCL.FormatCollapsedSkillCell(nil, "orange"))
      assert.are.equal("*", RCL.FormatCollapsedSkillCell(80240, "orange"))
    end)

    it("returns colored required skill over ***", function()
      local text = RCL.FormatCollapsedSkillCell(180, "orange")
      assert.are.equal("|cffff8040180|r/***", text)
      assert.are.equal("|cffffff00180|r/***", RCL.FormatCollapsedSkillCell(180, "yellow"))
      assert.are.equal("|cff40ff40180|r/***", RCL.FormatCollapsedSkillCell(180, "green"))
      assert.are.equal("|cff808080180|r/***", RCL.FormatCollapsedSkillCell(180, "gray"))
    end)
